...
そこで登場するのが、[CSS Paged Media Module Level 3](https://www.w3.org/TR/css-page-3/) というCSSの規格です。これは、文章が紙に印刷されたり、PDFになって、ページによって分割されたときにどのようにコンテンツが表示されるべきかを、CSSによって詳細に記述することができるようにするための規格です。

これを使えば、LaTeXのページ番号やヘッダー・フッターをHTML/CSSで再現することができます。しかし、この規格はまだ標準化団体が案として考えている段階なので、Chromeなどの各ブラウザには実装されていないません。
